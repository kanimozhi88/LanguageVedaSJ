...
const FacultyBatches = ({navigation}) => {
  ...
  const CourseApiCall = async () => {
    ...
    const response = await fetch(
      `${BASE_URL}/services/apexrest/RNFacultyBatchDisplay`,
      {
        method: 'POST',
        headers: new Headers({
  ...
  const renderItem = ({item, index}) => {
    return (
      <View style={{height: 151}}>
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('FacultyCourseBatch', {
              batchId: item?.Course_Offering_Id,
          ...
          <Text
            style={[
              {fontWeight: '500', color: '#000000', fontSize: 15, margin: 5},
            ]}>
            {item?.Course_Name}
          </Text>
          <View style={{flexDirection: 'row', marginTop: '35%', margin: 5}}>
            <View
              ...
              />
              <Text
                style={[{fontWeight: '400', color: '#F28018', fontSize: 12}]}>
