...
const AssessmentReport = ({navigation, route}) => {
  ...
  const AsssessmentReportApi = async () => {
    ...
    const response = await fetch(
      `${BASE_URL}/services/apexrest/RNStudentAssessmentScore`,
      {
        method: 'POST',
        headers: new Headers({
      ...
      <View>
            ...
            <View style={{alignSelf: 'center', marginTop: 10}}>
              {final?.totalMarksObtained ? (
                <Text
                  ...
                  {final?.totalMarksObtained} / {final?.totalMarks}
                </Text>
              ) : null}
              <Text style={{color: '#b0aeae', fontSize: 16, fontWeight: '400'}}>
                Total Score
              </Text>
            </View>
          </View>
        ...
        {final !== '' ? (
          <View style={{marginHorizontal: 25, marginTop: 20}}>
            ...
            <View
              style={{padding: 15, marginTop: 5, borderRadius: 5, width: 296}}>
              {final !== '' ? (
                <Text
