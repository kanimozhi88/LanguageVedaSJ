...
const FacultyStudentDetails = ({ route, navigation }) => {
  ...
  const { testId, assignmentTitle, Status, Name, batchName, TestDate, courseName } = route.params;
  ...
  const [showEssayMarks, setShowEssayMarks] = useState(false);
  ...
  const FctlyStudentDtls = async () => {
    ...
    const response = await fetch(`${BASE_URL}/services/apexrest/RNFacultyAssessmentEvaluation`, {
      method: 'POST',
      headers: new Headers({
  ...
  const updateStudentTestDetails = () => {
    if (description !== '') {
      StudentTestDetailsApi();
    } else {
      Alert.alert("Please enter the Feedback")
    }
  };

  const essayMarkUpdate = (quesId) => {
    if (essayMarks !== '') {
      EssayMarksUpdateApi(quesId);
    } else {
      Alert.alert("Please enter the Feedback")
    }
  }

  const StudentTestDetailsApi = async () => {
    let data = {};
    data.testId = testId;
  ...
  const EssayMarksUpdateApi = async (quesId) => {
    console.log("PAASSED QUESTIONID IS>>>>>>>>>", quesId);
    const existingArray = [];
    const testArray = {
      questionId: quesId,
      essayMark: essayMarks,
    }
    ...
    const token = await getAccessToken();
    const bearer = 'Bearer ' + token;
    const response = await fetch(`${BASE_URL}/services/apexrest/RNFacultyEssayMarkUpdate`, {
      method: 'PATCH',
      headers: new Headers({
        "Content-Type": "application/json",
        "Authorization": bearer
    ...
    let EssayMarks = await response.json()
  ...
  const QuestionScreen = ({ questionData, onNextPress, onPrevPress, selectedOption, uploadedImages, handleFileUpload }) => {
    ...
    if (questionData?.QuestionType === 'MCQ') {
      ...
      setShowEssayMarks(true)
    }

    return (
      <View style={[{ margin: 10, flex: 1 }]}>

        <Text style={{ color: "#696F79", fontSize: 16, fontWeight: "600", marginTop: 20, marginHorizontal: 5 }}>Question{currentIndex + 1}/{final?.length}</Text>
        <Text style={{ color: "#696F79", fontSize: 15, fontWeight: "400", marginTop: 20, marginHorizontal: 5 }}>{questionData?.QuestionText}</Text>
        {/* {(currentIndex+1) == final?.length ? setSubmitShow(true) : null} */}
        {/* Render options for MCQ or textarea for Essay Question */}
        {questionData?.QuestionType === 'MCQ' && crtAns !== null && userAns !== null ?
        ...
        <View style={{ flexDirection: "row", width: "90%", marginTop: "15%", marginLeft: "60%" }}>
          <TouchableOpacity
            // style={{backgroundColor: currentIndex === 0 ? "#999999":"#F38216",}}
          ...
          <TouchableOpacity
            ...
            <Text style={{ marginLeft: 15, color: (currentIndex + 1) === final?.length ? "#7E7E7E" : "#000000", fontSize: 14, fontWeight: "400", alignSelf: "center" }}>Next {'\u003E'}</Text>
          </TouchableOpacity>
        </View>
...
{showEssayMarks && Status !== "Completed" ?
        <View style={{ marginTop: 20, marginHorizontal: 25 }}>
          ...
          <View style={{ flexDirection: "row", width: "100%", backgroundColor: "#F5F7FB", padding: 15, marginTop: 10 }}>
            ...
            <TouchableOpacity
              ...
              style={{ width: "30%", backgroundColor: "#F38216", marginLeft: "20%", borderRadius: 10, padding: 15 }}>
              <Text style={{ color: "white", fontSize: 14, fontWeight: "500", alignSelf: "center", }}>Update</Text>
            </TouchableOpacity>
          </View>
        </View>
        : null}
        ...
        <View style={{ alignItems: "center", marginBottom: "30%", marginHorizontal: 25, marginTop: 20 }}>
          ...
          {/* {final !== '' && Status === "Completed" ? */}
          <TouchableOpacity
            onPress={() => updateStudentTestDetails()}
