...
const StudentCourseAssignment = ({batchId, courseName}) => {
  ...
  const renderItem = ({item, index}) => {
    return (
      <TouchableOpacity
        onPress={() => {
          navigation.navigate('StudentAssignmentUpload', {testId: item.id});
        }}
        style={{
          width: '99%',
  ...
  return (
    ...
    <View style={{flex: 1}}>
      ...
      <Text
        style={{color: '#F38216', fontSize: 20, fontWeight: '600', margin: 30}}>
        {courseName}
      </Text>
      {final !== '' && final !== undefined ? (
        <View style={{height: 280}}>
          <FlatList data={final} renderItem={renderItem} />
        </View>
      ) : null}
    </View>
  );
