...
const StudentCourseCurriculum = ({batchId, courseName}) => {
  ...
  const StudentCurriculumApiCall = async () => {
    ...
    const response = await fetch(
      `${BASE_URL}/services/apexrest/lessonPlanExecutions`,
      {
        method: 'POST',
        headers: new Headers({
  ...
  const totalProgress = (final?.lessonPlanExecutions ?? []).reduce(
    (total, obj) => total + (obj.Progress || 0),
    0,
  );
  const averageProgress =
    totalProgress / Math.max(final?.lessonPlanExecutions?.length, 1);
  ...
  useEffect(() => {
    ...
    const step =
      (statusTargetFill - currentStatusFill) *
      (statusIntervalDuration / statusDuration);
  ...
  const renderItem = ({item, index}) => {
    return (
      <View
        ...
        <Text
          style={{color: 'black', fontSize: 16, fontWeight: '500', margin: 10}}>
          {item.TopicName}
        </Text>
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <Text
        style={{color: '#F38216', fontSize: 16, fontWeight: '600', margin: 20}}>
        {courseName}
      </Text>
      <View style={{height: 280, marginBottom: 20}}>
        <FlatList data={final?.lessonPlanExecutions} renderItem={renderItem} />
      </View>
    </View>
  );
};
